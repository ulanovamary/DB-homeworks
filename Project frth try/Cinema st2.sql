...
SELECT DISTINCT g.name, 
  ...
  COUNT(pfc.film_crew_id) OVER (PARTITION BY pfc.project_id) AS film_crew_persons_,
  a.name AS acc_name,
  COUNT(projects.id) OVER (PARTITION BY projects.access_id) AS quantity_films_in_acc
  FROM genres g
      LEFT JOIN projects 
        ON g.id = projects.genre_id
      JOIN project_film_crew pfc 
     	ON projects.id=pfc.project_id
     JOIN accesses a 
...
FROM projects p
JOIN project_film_crew pfc 
	ON p.id = pfc.project_id
JOIN film_crew fc
ON fc.id = pfc.film_crew_id
JOIN professions pr
